...
document.addEventListener('DOMContentLoaded', function () {
    // --- Common Fields HTML ---
    // This HTML will be injected into each form tab for consistency.
    const commonFieldsHTML = `
        <div class="row justify-content-center">
            <div class="col-md-6">
                <label class="form-label f-w-500">Spam Check*</label>
                <div class="input-group">
                    <span class="input-group-text spam-question"></span>
                    <input type="text" class="form-control spam-answer" required>
                </div>
            </div>
        </div>
    ...
    // Inject the common fields into all form tabs
    document.querySelectorAll('.common-fields').forEach(fieldContainer => {
        fieldContainer.innerHTML = commonFieldsHTML;
    });

    // --- Membership Fee Logic ---
    // Auto-fill amount when membership type is selected
    const membershipSelect = document.querySelector('select[name="membership_plan"]');
    if (membershipSelect) {
        membershipSelect.addEventListener('change', function() {
            const selectedValue = this.value;
            const amountInput = this.closest('form').querySelector('input[name="amount"]');
            if (selectedValue) {
                const parts = selectedValue.split('|'); // e.g., "Life Member|10000"
    ...
    // --- Spam Check Logic for All Forms ---
    // This function sets a new spam question for a given form
    const setSpamQuestion = (form) => {
        const num1 = Math.ceil(Math.random() * 10);
        const num2 = Math.ceil(Math.random() * 10);
        form.querySelector('.spam-question').textContent = `${num1} + ${num2} =`;
        form.dataset.spamAnswer = num1 + num2;
    };
    ...
    document.querySelectorAll('#payment-tab-content form').forEach(form => {
        ...
        form.addEventListener('submit', function(e) {
            ...
            const userAnswer = parseInt(this.querySelector('.spam-answer').value, 10);
            if (userAnswer !== spamAnswer) {
                e.preventDefault();
                alert('Spam check failed! Please provide the correct answer.');
            }
        });
    });

    // Reset spam question when a tab is shown
    const paymentTabs = document.querySelectorAll('#payment-tab-nav button[data-bs-toggle="tab"]');
    paymentTabs.forEach(tab => {
        tab.addEventListener('shown.bs.tab', event => {
            const targetPaneId = event.target.getAttribute('data-bs-target');
            const targetForm = document.querySelector(targetPaneId + ' form');
            if (targetForm) {
                setSpamQuestion(targetForm);
            }
        });
    });
});
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tabBoxes.forEach(box => {
        box.addEventListener('click', function() {
            ...
            tabBoxes.forEach(b => b.classList.remove('current'));
    ...
    const commonFieldsHTML = `
        ...
        <div class="row justify-content-center mt-3">
            <div class="col-md-6">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" value="" required>
                    <label class="form-check-label">I agree to the <a href="/terms" target="_blank">Terms & Conditions</a>.</label>
                </div>
            </div>
        </div>
    ...
    // --- Membership Fee Dependent Field Logic ---
    const membershipSelect = document.querySelector('select[name="membership_plan"]');
    if (membershipSelect) {
        membershipSelect.addEventListener('change', function() {
            const selectedValue = this.value;
            const amountInput = this.closest('form').querySelector('input[name="amount"]');
            amountInput.value = selectedValue ? selectedValue.split('|')[1] : '';
        });
    }

    // --- Spam Check Logic for All Forms ---
    const setupSpamCheck = (form) => {
        const num1 = Math.ceil(Math.random() * 10);
        const num2 = Math.ceil(Math.random() * 10);
        form.querySelector('.spam-question').textContent = `${num1} + ${num2} =`;
        form.dataset.spamAnswer = num1 + num2;
    ...
    document.querySelectorAll('#paymentModal form').forEach(form => {
        setupSpamCheck(form);
    });
});
