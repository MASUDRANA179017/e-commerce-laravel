...
@section('content')
                    ...
                    async function sendProduct(btn) {
                        ...
                        async function applyPrimaryConfig(isInitialLoad = false) {
                            ...
                            if (setSel) {
                                ...
                                const shouldSkip = isInitialLoad && SERVER_DATA.isEdit && SERVER_DATA.product
                                    ?.attribute_set_id;
  ...
  <ul>${list || '<li class="small-muted">No attributes.</li>'}</ul>`;
                ...
                //     Description: ${( $('#metaDesc')?.value || $('#shortDesc')?.value || '' ).slice(0,160)}`);
                        ...
                        /* ===== Init ===== */
                        async function init() {
                            openTab(location.hash && document.querySelector(location.hash) ? location.hash :
                                '#tab-basic');
                            try {
                                await loadBootstrap();
                        ...
                        @if (isset($isEdit) && $isEdit && isset($product) && $product)
                            (async function loadEditData() {
                                ...
                                const wantRule =
                                    '{{ isset($product->variant_rule_id) ? $product->variant_rule_id : '' }}';

                                // Wait until dropdowns are populated
                                const waitForSelects = async () => {
                                    const hasSets = () => setSel && setSel.options && setSel.options
                                        .length > 1;
                                    const hasRulesReady = () => varRuleSel && varRuleSel.options &&
                                        varRuleSel
                                        .options.length >= 1;
                                    let tries = 0;
                                ...
                                let tries2 = 0;
                                while (varRuleSel && (!varRuleSel.options || varRuleSel.options.length <= 1) &&
                                    tries2 <
                                    40) {
                                    await new Promise(r => setTimeout(r, 100));
                                    tries2++;
                                ...
                                $$('.term-check').forEach(c => {
                                    ...
                                    if (Array.isArray(list) && list.some(x => String(x) === tid)) c
                                        .checked = true;
                                });
