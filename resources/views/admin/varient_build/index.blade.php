...
@section('content')
    ...
    <!-- Saved Variant Sets Section -->
    <div class="saved-sets-section">
        <div class="sets-header" id="toggleSavedSets">
            <div class="d-flex align-items-center gap-2">
                <i class="bx bx-package" style="font-size:1.25rem"></i>
                <h6 class="mb-0">Saved Variant Sets</h6>
                <span class="badge bg-primary" id="savedSetsCount">0</span>
            </div>
            <i class="bx bx-chevron-down" id="savedSetsIcon"></i>
        </div>
        <div class="sets-list d-none" id="savedSetsList">
            <div class="text-center py-4 text-muted" id="noSetsMessage">
                <i class="bx bx-info-circle" style="font-size:2rem"></i>
                <p class="mb-0 mt-2">No saved variant sets yet</p>
            </div>
        </div>
    </div>
            ...
            <div class="card sticky-side">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-hint">Will be used when generating SKUs (e.g., <span
                                class="kbd">T-SHIRT-RED-M</span>)</div>
                    </div>
                    ...
                    <div class="d-grid gap-2">
                        <button class="create-btn-base" id="btnGenerate"><i class="bx bx-grid-alt me-1"></i>Generate
                            Variants</button>
                        <button class="create-btn-white" id="btnReset"><i class="bx bx-reset me-1"></i>Reset</button>
                        <button class="create-btn-success" id="btnSave"><i class="bx bx-save me-1"></i>Save Variant
                            Set</button>
                    </div>
                </div>
            </div>
                    ...
                    <div class="d-flex align-items-center justify-content-between" id="variantHeader">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <div class="d-flex gap-2" data-actions-view>
                                <button class="icon-btn" data-action="edit-card-title" title="Edit title"><i
                                        class="bx bx-edit-alt"></i></button>
                            </div>
                            <div class="d-flex gap-2 d-none" data-actions-edit>
                                <button class="btn btn-primary btn-sm" data-action="save-card-title"><i
                                        class="bx bx-save me-1"></i>Save</button>
                                <button class="icon-btn" data-action="cancel-card-title" title="Cancel"><i
                                        class="bx bx-x"></i></button>
                            </div>
                            <span class="chip"><i class="bx bx-purchase-tag-alt"></i> <span id="variantCount">0</span>
                                variants</span>
                        </div>
                    </div>
                    ...
                    <div id="variantPanel">
                        ...
                        <div class="row g-3 align-items-start">
                            <div class="col-md-7">
                                ...
                                <div class="form-hint mt-1">Opens the modals/actions already present in your codebase.
                                </div>
                            </div>
                            <div class="col-md-5">
                                ...
                                <div class="mt-3 opt-note">
                                    Price & Quantity will be managed from Inventory/Purchase; Images from Product Add —
                                    hidden here.
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-2 d-flex align-items-center justify-content-between">
                            ...
                            <div id="skuError" class="text-danger small d-none"><i
                                    class="bx bx-error-circle me-1"></i>Duplicate SKUs detected.</div>
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts')
    <script>
        ...
        function renderRulesPreview() {
            const ul = $('#rulesPreview');
            ul.innerHTML = "";
            const mediaSel = $$('#mediaRules option:checked').map(o => o.textContent);
            const rules = [];
            if (mediaSel.length) {
                rules.push(`<li><span class="pill"><i class='bx bx-image'></i> ${mediaSel.join(', ')}</span></li>`);
            }
            if ($('#ruleUnique').checked) {
                rules.push(`<li><span class="pill"><i class='bx bx-purchase-tag'></i> Require unique SKU</span></li>`);
            }
            if ($('#rulePrice').checked) {
                rules.push(`<li><span class="pill"><i class='bx bx-money'></i> Variant wise Price</span></li>`);
            }
        ...
        function renderVariantsTable() {
            ...
            if (!variants.length) {
                ...
                tr.innerHTML =
                    `<td colspan="4"><div class="d-flex align-items-center gap-2"><i class="bx bx-info-circle"></i><span>No variants yet. Click <b>Generate Variants</b>.</span></div></td>`;
      ...
      `;
        ...
        document.getElementById('variantHeader').addEventListener('click', (e) => {
            ...
            } else if (action === 'cancel-card-title') {
        ...
        document.getElementById('btnGenerate').addEventListener('click', () => {
            ...
            if (!set) {
                return alert('Please select an Attribute Set.');
            }
        ...
        document.getElementById('btnSave').addEventListener('click', () => {
            ...
            const payload = {
                ...
                variants: variants.map(v => ({
                    ...
                    options: v.options.map(o => ({
                        attribute_id: o.attrId,
                        attribute_name: o.attrName,
                        term_id: o.termId,
            ...
            fetch('{{ route('admin.users.varient-build.store') }}', {
                ...
                variants = [];
                renderVariantsTable();
                loadSavedVariantSets(); // Reload the saved sets list
        ...
        renderRulesPreview();
        renderVariantsTable();
        loadSavedVariantSets();

        /** =========================
         * Saved Variant Sets Section
         * ========================= */
        const toggleSavedSets = document.getElementById('toggleSavedSets');
        const savedSetsList = document.getElementById('savedSetsList');
        const savedSetsIcon = document.getElementById('savedSetsIcon');

        toggleSavedSets?.addEventListener('click', () => {
            savedSetsList?.classList.toggle('d-none');
            savedSetsIcon?.classList.toggle('rotate-180');
        });

        function loadSavedVariantSets() {
            fetch('{{ route('admin.users.varient-build.index') }}?ajax=1', {
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                ...
                .then(data => {
                    console.log('Loaded variant sets:', data);
                    const sets = data.variant_sets || [];
                    const countBadge = document.getElementById('savedSetsCount');
                    const list = document.getElementById('savedSetsList');
                    const noMsg = document.getElementById('noSetsMessage');

                    if (countBadge) countBadge.textContent = sets.length;

                    if (sets.length === 0) {
                        if (noMsg) noMsg.classList.remove('d-none');
                        if (list) {
                            list.innerHTML =
                                '<div class="text-center py-4 text-muted" id="noSetsMessage"><i class="bx bx-info-circle" style="font-size:2rem"></i><p class="mb-0 mt-2">No saved variant sets yet</p></div>';
                        }
                        return;
                    }
      ...
      <div class="set-item">
        <div class="set-name">${escapeHtml(set.name || 'Unnamed Set')}</div>
        <div class="set-meta">
          <span><i class="bx bx-grid-alt"></i> ${set.variants_count || 0} variants</span>
          ${set.business_category ? `<span class="badge badge-soft">${escapeHtml(set.business_category.name)}</span>` : ''}
        </div>
        <div class="d-flex gap-2">
          <button class="icon-btn" onclick="viewVariantSet(${set.id})" title="View">
            <i class="bx bx-show"></i>
          </button>
          <button class="icon-btn" onclick="editVariantSet(${set.id})" title="Edit">
            <i class="bx bx-edit-alt"></i>
          </button>
          <button class="icon-btn text-danger" onclick="deleteVariantSet(${set.id})" title="Delete">
    ...
    `).join('');
                    ...
                    // Auto-expand if there are sets
                    if (sets.length > 0 && savedSetsList && savedSetsList.classList.contains('d-none')) {
                        savedSetsList.classList.remove('d-none');
                        if (savedSetsIcon) savedSetsIcon.classList.add('rotate-180');
                    }
                })
                .catch(err => {
                    console.error('Failed to load saved variant sets:', err);
                    const list = document.getElementById('savedSetsList');
                    if (list) {
                        list.innerHTML =
                            '<div class="text-center py-3 text-danger"><i class="bx bx-error-circle"></i> Failed to load variant sets</div>';
        ...
        function viewVariantSet(id) {
            // Fetch the variant set details
            fetch(`{{ url('admin/varient-build') }}/${id}/show`, {
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                ...
                .then(res => res.json())
                .then(data => {
                    if (data && data.variant_set) {
                        const set = data.variant_set;
                        let variantsHtml = '';
                        if (set.variants && set.variants.length > 0) {
                            variantsHtml = set.variants.map((v, idx) => `
          <tr>
            <td>${idx + 1}</td>
            <td>${v.label}</td>
            <td>${v.sku}</td>
        ...
        `).join('');
                            ...
                            variantsHtml = '<tr><td colspan="3" class="text-center text-muted">No variants</td></tr>';
                        }

                        const content = `
        <div class="mb-3">
          <label class="small-muted">Set Name</label>
          <p class="fw-semibold">${escapeHtml(set.name)}</p>
        </div>
        <div class="mb-3">
          <label class="small-muted">SKU Prefix</label>
          <p class="fw-semibold">${escapeHtml(set.sku_prefix || '-')}</p>
        </div>
        <div class="mb-3">
          <label class="small-muted">Variants Count</label>
          <p class="fw-semibold">${set.variants_count || 0}</p>
        </div>
        <div class="mb-3">
          <label class="small-muted">Variants</label>
      ...
      `;
                        ...
                        if (window.Swal) {
                            ...
                            alert('Variant Set: ' + set.name + '\nVariants: ' + set.variants_count);
                        }
                    }
                })
                .catch(err => {
                    console.error('Failed to load variant set:', err);
                    if (window.toastr) toastr.error('Failed to load variant set');
                });
        }

        function editVariantSet(id) {
            // Fetch the variant set details
            fetch(`{{ url('admin/varient-build') }}/${id}/show`, {
                    headers: {
                        'Accept': 'application/json',
                ...
                .then(data => {
                    if (data && data.variant_set) {
                        ...
                        document.getElementById('skuPrefix').value = set.sku_prefix || '';

                        // Set business category if available
                        if (set.business_category && set.business_category.id) {
                            document.getElementById('bizSelect').value = set.business_category.id;
                            document.getElementById('bizSelect').dispatchEvent(new Event('change'));
                        }
                        ...
                        if (window.toastr) toastr.info('Variant set loaded. You can now edit and save.');
                    }
                })
                .catch(err => {
                    console.error('Failed to load variant set:', err);
        ...
        function deleteVariantSet(id) {
            if (!window.Swal) {
                if (!confirm('Are you sure you want to delete this variant set?')) return;
                performDelete(id);
                return;
            }
            ...
            Swal.fire({
                ...
                confirmButtonText: '<i class="bx bx-trash me-2"></i>Yes, delete it!',
                cancelButtonText: '<i class="bx bx-x me-2"></i>Cancel',
                reverseButtons: true
            }).then(result => {
        ...
        function performDelete(id) {
            fetch(`{{ url('admin/varient-build') }}/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest',
                ...
                .then(data => {
                    if (window.Swal) {
                        Swal.fire('Deleted!', data.message || 'Variant set deleted successfully', 'success');
                    } else if (window.toastr) {
                        toastr.success(data.message || 'Variant set deleted');
                    }
                    loadSavedVariantSets();
                })
                .catch(err => {
                    ...
                    if (window.Swal) {
                        Swal.fire('Error!', 'Failed to delete variant set', 'error');
                    } else if (window.toastr) {
                        toastr.error('Failed to delete variant set');
                    }
                });
        }
    </script>
