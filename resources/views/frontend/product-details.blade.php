        ...
        <div class="row">
                ...
                <div class="product-images">
                    ...
                    <div class="main-image mb-3 position-relative overflow-hidden rounded-4" style="background: #f8f9fa;">
                        @if(isset($product->images) && $product->images->count() > 0)
                            <img src="{{ asset('storage/' . $product->images->first()->image) }}" alt="{{ $product->name }}" class="img-fluid w-100" id="mainProductImage" style="max-height: 500px; object-fit: contain;">
                        @else
                            <img src="{{ asset('storage/product/' . ($product->thumbnail ?? 'default.png')) }}" alt="{{ $product->name }}" class="img-fluid w-100" id="mainProductImage" style="max-height: 500px; object-fit: contain;" onerror="this.src='https://via.placeholder.com/500x500?text=No+Image'">
                        @endif
                    ...
                    <div class="thumbnail-gallery d-flex gap-2 flex-wrap">
                        ...
                        <div class="thumbnail-item {{ $index == 0 ? 'active' : '' }}" style="width: 80px; height: 80px; cursor: pointer; border: 2px solid {{ $index == 0 ? '#0496ff' : '#eee' }}; border-radius: 8px; overflow: hidden;">
                            <img src="{{ asset('storage/' . $image->image) }}" alt="{{ $product->name }}" class="img-fluid w-100 h-100" style="object-fit: cover;" onclick="changeMainImage(this, '{{ asset('storage/' . $image->image) }}')">
                        </div>
                        @endforeach
                    </div>
                    @endif
            ...
            <div class="col-12 col-lg-7">
                <div class="product-info" data-aos="fade-up">
                    <!-- Product Title -->
                    <h1 class="product-title mb-3" style="font-size: 32px; font-weight: 700;">{{ $product->name ?? 'Product Name' }}</h1>
                    ...
                    <div class="product-price mb-4">
                        @if(isset($product->sale_price) && $product->sale_price && $product->sale_price < $product->price)
                            <span class="current-price text-danger" style="font-size: 36px; font-weight: 700;">${{ number_format($product->sale_price, 2) }}</span>
                            <span class="original-price text-muted text-decoration-line-through ms-2" style="font-size: 24px;">${{ number_format($product->price, 2) }}</span>
                            <span class="badge bg-danger ms-2">Save ${{ number_format($product->price - $product->sale_price, 2) }}</span>
                        @else
                            <span class="current-price" style="font-size: 36px; font-weight: 700; color: #0496ff;">${{ number_format($product->price ?? 0, 2) }}</span>
                        @endif
                    </div>
                    ...
                    <form action="{{ route('cart.add') }}" method="POST" id="add-to-cart-form">
                        ...
                        <div class="quantity-selector d-flex align-items-center gap-3 mb-4">
                            ...
                            <div class="input-group" style="width: 140px;">
                                ...
                                <input type="number" name="quantity" id="quantity" class="form-control text-center" value="1" min="1" max="{{ $product->stock ?? 99 }}">
                        ...
                        <div class="action-buttons d-flex flex-wrap gap-3 mb-4">
                            <button type="submit" class="btn btn-success btn-lg px-5" {{ ($product->stock ?? 0) <= 0 ? 'disabled' : '' }}>
                                <i class="fa-solid fa-shopping-cart me-2"></i> Add to Cart
                            </button>
                            <button type="button" class="btn btn-primary btn-lg px-5 buy-now-btn" {{ ($product->stock ?? 0) <= 0 ? 'disabled' : '' }}>
                    ...
                    <div class="social-share pt-3 border-top">
                        ...
                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($product->name ?? '') }}" target="_blank" class="btn btn-sm btn-outline-info me-2">
                            <i class="fab fa-twitter"></i>
                        </a>
                        <a href="https://wa.me/?text={{ urlencode(($product->name ?? '') . ' ' . request()->url()) }}" target="_blank" class="btn btn-sm btn-outline-success me-2">
                            <i class="fab fa-whatsapp"></i>
                        </a>
                        <a href="https://pinterest.com/pin/create/button/?url={{ urlencode(request()->url()) }}&description={{ urlencode($product->name ?? '') }}" target="_blank" class="btn btn-sm btn-outline-danger">
                ...
                <div class="tab-content" id="productTabsContent">
                    ...
                    <div class="tab-pane fade" id="specifications" role="tabpanel">
                        <table class="table table-striped">
                            <tbody>
                                <tr><th width="200">SKU</th><td>{{ $product->sku ?? 'N/A' }}</td></tr>
